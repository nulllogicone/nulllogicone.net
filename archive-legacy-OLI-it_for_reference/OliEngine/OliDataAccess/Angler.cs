...
namespace OliEngine.OliDataAccess
{
    ...
    public class Angler : AnglerDataSet
    {
        ...
        public Angler()
        {
            ...
            lad = new SqlDataAdapter("SELECT * FROM oli.L�cher", con);
...
//			aad.Fill(this.Angler);
        ...
        public Angler(Guid aguid)
        {
            ...
            lad = new SqlDataAdapter("SELECT * FROM oli.L�cher WHERE AnglerGuid='" + aguid + "'", con);
            SqlCommandBuilder lcb = new SqlCommandBuilder(lad);
            lad.Fill(L�cher);
        }
        ...
        public int UpdateL�cher()
        {
            return (lad.Update(L�cher));
        }
        ...
        public void ClearLoecher()
        {
            if (AnglerRow != null)
            {
                using (var con = OliCommon.OLIsConnection)
                {
                    ...
                    SqlCommand cmd = new SqlCommand();
                    cmd.CommandText = "DELETE FROM L�cher WHERE AnglerGuid='" + AnglerRow.AnglerGuid + "'";
                    cmd.Connection = con;
        ...
        public static Angler parseAnglerRDF(string anglerInput)
        {
            ...
            st = parser.GetStatements(anglerStatement.Subject.Value, "http://nulllogicone.net/schema.rdfs#anglerStamm",
                                      null);
            if (st.Count != 1)
                throw new Exception("Keiner oder mehrere St�mme gefunden. Ein Angler kann nur von einem Stamm sein");
    ...
    /// <summary>
    ///     Kein Angler Exception - weil die Angler gel�scht werden k�nnen,
    ///     kommt es immer mal wieder vor, da� einer nicht gefunden wird:
