...
{
	/// <summary>
	/// Zusammendfassende Beschreibung f�r Abrechnen.
	/// </summary>
	public class Abrechnung
	{
		...
		public bool Abrechnen(out string message)
		{
			...
			// Wurzelreihe (f�r bezahlt und closed)
			...
			// ohne Antworten geht Geld zur�ck
			if(tl.TopLab.Count == 0)
			{
				// Stamm Restgeld zur�ck
				...
				// Nachricht und weg !!!!!!!!!
				message = "Keine Antworten also " + rest.ToString() + " zur�ck <br>";
			...
			// Ich habe Bewertungen vergeben also mal zusammenz�hlen:
			short gesamtToll = 0;
			...
			if(gesamtToll < 100)
			{
				...
				stamm.UpdateStamm();
				message += stamm.StammRow.Stamm + " bekommt " + (rest * (100-gesamtToll) / 100).ToString() + " zur�ck<br>";
				// vom Rest abziehen
				rest -= rest * (100-gesamtToll) / 100;
			}
			else
			{
				message += "Bewertung �ber 100 <br>";
			}
		...
		private bool BezahlAntwort(Guid tguid, decimal betrag)
		{
			...
			string text = "Sie haben f�r Ihre Antwort: <br>";
