...
namespace OliWeb.Controls.AjaxWortraum
{
    ...
    public partial class AjaxWortraumControlFlip : UserControl
    {
        ...
        {
            Utility.RegisterTypeForAjax(typeof (AjaxWortraumControlFlip));
            if (!Markierbar)
            {
        ...
        private string makeNetz(string nguidstr, int ebene, bool onlymarked)
        {
            ...
            {
                sb.Append("<img style=\"float:left; width:50px;\" src=\"" + OliUtil.MakeImageSrc( n.NetzRow.Datei )+
                          "\"/>");
            }
        ...
        private string makeKnoten(KnotenDataSet.KnotenRow kr, int ebene, bool onlymarked)
        {
            ...
            if (Common.CurrentCulture.TwoLetterISOLanguageName == "en")
            {
                sb.Append((kr.Isen_nameNull() ? kr.Knoten : kr.en_name) + "&nbsp;");
            }
            else
            {
                sb.Append((kr.Knoten) + "&nbsp;");
        ...
        private string makeBaum(KnotenDataSet.KnotenRow lastKnoten, string bguidstr, int ebene, bool onlymarked)
        {
            ...
            {
                sb.Append("<img style=\"float:left; width:50px;\" src=\"" + OliUtil.MakeImageSrc(b.BaumRow.Datei) +
                          "\"/>");
            }
            // Tooltipp + Text
            if (Common.CurrentCulture.TwoLetterISOLanguageName == "en")
